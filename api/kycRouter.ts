...
async function audit(
  customerRef: string,
  action: "DOC_ATTEMPT" | "STATUS_CHANGE",
  actorUserId: number,
...
/** Number of DOC_ATTEMPT audit rows for the customer since UTC midnight. */
async function docAttemptsToday(customerRef: string): Promise<number> {
  ...
  const rows = await getDb()
    ...
    .where(
      and(
        ...
        eq(identityVaultAudit.action, "DOC_ATTEMPT"),
...
export const kycRouter = createRouter({
  ...
  status: authedQuery.query(async ({ ctx }) => {
    ...
    const vaultRows = await getDb()
      .select({ status: identityVault.status })
      .from(identityVault)
      .where(eq(identityVault.customerRef, customerRef))
      .limit(1);
    const attemptsToday = await docAttemptsToday(customerRef);
  ...
  submitDoc: authedQuery
    ...
    .mutation(async ({ ctx, input }): Promise<SubmitDocResult> => {
      ...
      const attemptsBefore = await docAttemptsToday(customerRef);
      ...
      const compact = input.imageBase64
        .replace(/^data:[^;,]*(?:;base64)?,/, "")
        .replace(/\s+/g, "");
      ...
      if (
        ...
        imageBuffer.length > MAX_DECODED_BYTES
      ) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Decoded image must be larger than 0 bytes and at most 10 MB",
        });
      }
